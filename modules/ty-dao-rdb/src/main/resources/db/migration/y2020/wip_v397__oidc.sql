...
create table identity_providers_t(
  ...
  constraint identityproviders_c_opauthorizationurl_len check (length(idp_authorization_url_c) between 1 and 500),
  constraint identityproviders_c_opaccesstokenurl_len check (length(idp_access_token_url_c) between 1 and 500),
  constraint identityproviders_c_opuserinfourl_len check (length(idp_user_info_url_c) between 1 and 500),
  constraint identityproviders_c_oplogouturl_len check (length(idp_logout_url_c) between 1 and 500),
  constraint identityproviders_c_opclientid_len check (length(idp_client_id_c) between 1 and 500),
  constraint identityproviders_c_opclientsecret_len check (length(idp_client_secret_c) between 1 and 500),
  constraint identityproviders_c_opissuer_len check (length(idp_issuer_c) between 1 and 200),
  constraint identityproviders_c_opscopes_len check (length(idp_scopes_c) between 1 and 500),
